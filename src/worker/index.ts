  ...
  "/break": () =>
    shell(
      ...
      <div class="card" style="margin-top:1rem">
        ...
        <div style="margin-top:1rem">${SPRIG_SVG}</div>
      </div>`,
      `document.getElementById('break-go').onclick=()=>{let t=60;const el=document.getElementById('break-timer');el.textContent=t+'s';const i=setInterval(()=>{t--;el.textContent=t+'s';if(t<=0){clearInterval(i);el.textContent='Done!';window.GiggleSprout.laugh(2);}},1000);};document.getElementById('break-done').onclick=()=>window.GiggleSprout.celebrate();`
    ),
  "/gym": () =>
    shell(
      "Humor Gym",
      `<h2>Humor Gym</h2>
      <p class="lead">Micro-drills to sharpen timing, observation, and delivery.</p>
      <div class="grid3">
        <div class="card"><h3>Observation</h3><p>Spot 3 absurd details in 30s.</p><button class="btn" onclick="window.GiggleSprout.laugh(1)">Start drill</button></div>
        <div class="card"><h3>Timing</h3><p>Deliver punchline after pause.</p><button class="btn" onclick="window.GiggleSprout.grow()">Start drill</button></div>
        <div class="card"><h3>Delivery</h3><p>Try 3 voices on same line.</p><button class="btn" onclick="window.GiggleSprout.celebrate()">Start drill</button></div>
      </div>
      <div style="margin-top:1.5rem;text-align:center">${SPRIG_SVG}</div>`
    ),
  "/roleplay": () =>
    shell(
      "Roleplay",
      `<h2>Roleplay Coach</h2>
      <div class="grid3">${Object.keys(SCENARIOS).map(k=>`<div class="card"><a href="/roleplay/${k}">${SCENARIOS[k].title}</a></div>`).join('')}</div>`
    ),
};
...
function handleRequest(request: Request, env: Env): Response {
  ...
  if (PAGES[p]) return new Response(PAGES[p](), { headers: { "content-type": "text/html; charset=utf-8" } });
  if (p.startsWith("/roleplay/")) {
    const key = p.split("/")[2] as keyof typeof SCENARIOS;
    if (SCENARIOS[key]) {
      const s = SCENARIOS[key];
      return new Response(shell(s.title, `<h2>${s.title}</h2><div class="card"><pre>${s.openers.join("\n\n")}</pre></div>`), { headers: { "content-type": "text/html; charset=utf-8" } });
    }
  }
